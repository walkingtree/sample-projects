        ...
        if (config) {
            if (!scrollView) {
                ...
                if (!Ext.feature.has.ProperHBoxStretching) {
                    ...
                    config.translatable = Ext.factory(config.translatable || scroller.config.translatable, Ext.util.Translatable);
                    direction = config.direction || 'auto';

                    if (config.translatable && config.translatable.type != 'scrollposition' && direction !== 'vertical') {
                        extraWrap = scrollerElement.wrap();
                        extraWrap.setStyle({position: 'absolute', minWidth: '100%'});

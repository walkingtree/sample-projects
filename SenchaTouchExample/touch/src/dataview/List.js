...
Ext.define('Ext.dataview.List', {
        ...
        for (i = 0; i < ln; i++) {
            ...
            if (itemIndex !== null) {
                ...
                if (headerIndices && headerIndices[itemIndex]) {
                    if (!me.headerHeight) {
                        me.headerHeight = parseInt(item.getHeader().renderElement.getHeight(), 10);
                    }
    ...
    createItem: function(config) {
        ...
        if (!infinite) {
            ...
            if (!header.getTranslatable()) {
                header.setTranslatable(true);
            }
        ...
        if (info.grouped) {
            if (headerIndices[index]) {
                ...
                if (!infinite) {
                    ...
                    if (!me.headerHeight) {
                        me.headerHeight = parseInt(header.renderElement.getHeight(), 10);
                    }
                }
            }
    ...
    onContainerResize: function(container, size) {
        this.setItemsCount(Math.ceil(size.height / this.getItemMap().getMinimumHeight()) + 1);
    },
    ...
    refreshScroller: function() {
        ...
        if (me.isPainted()) {
            ...
            } else if (me.getGrouped() && me.getPinHeaders()) {
                me.updateHeaderMap();
            }
        }
    },
    ...
    onStoreClear: function() {
        if (this.pinnedHeader) {
            this.pinnedHeader.translate(0, -10000);
        }
        this.showEmptyText();
    ...
    onScrollDockItemResize: function(item, size) {
        Ext.getCmp(item.id).$scrollDockHeight = size.height;

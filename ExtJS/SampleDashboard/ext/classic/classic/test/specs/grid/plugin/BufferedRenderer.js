describe('Ext.grid.plugin.BufferedRenderer', function () {
    ...
    describe("basic functionality with a buffered store", function() {
        ...
        it('should maintain synchronization when scrolling locked, variable row height grid with keyboard', function() {
            ...
            waitsFor(function() {
                ...
                if (normalRows.startIndex === lockedRows.startIndex) {
                    jasmine.fireKeyEvent(Ext.Element.getActiveElement(), 'keydown', Ext.EventObject.PAGE_DOWN);
                }
            ...
            runs(function() {
                ...
                // Scroll range must still be in sync
                expect(lockedView.getScrollable().getSize().y).toBe(normalView.getScrollable().getSize().y);

                // Row height must still be in sync
                expect(lockedView.bufferedRenderer.rowHeight).toBe(normalView.bufferedRenderer.rowHeight);
        ...
        describe('expanding/collapsing', function () {
            it('should always render the view nodes when expanding', function () {
                ...
                runs(function () {
                    ...
                    // Some new nodes should have been immediately rendered to allow scrolling to bring them into view
                    expect(nodeCache.endIndex).toBeGreaterThan(99);
                });
    ...
    describe('filtering the store', function () {
        it('should reset the cached position so the grid-item-container is at the top of the view', function () {
            var Hobbit, store;
            ...
            runs(function () {
                ...
                expect(plugin.bodyTop).toBeLE(0);
                expect(plugin.position).toBeLE(0);
